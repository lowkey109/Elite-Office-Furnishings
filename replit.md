...
- **Trading AI Monitor** (`/admin/trading-monitor`): Completely separate paper trading observation panel for crypto (BTC/USD, ETH/USD) and gold (XAUUSD). 5 sections: State Cards (9: Mode, Regime, Last Decision, Total Trades, Win Rate, Drawdown, Open Positions, Best Strategy, Data Quality), Decision Feed (expandable with thesis/risk/payload), Open Positions, Performance (PnL chart + stats), Recent Outcomes (green/red). Read-only, auto-refresh 5s, paper mode only. Backend: `GET /api/admin/trading/monitor` (admin-auth protected). Service: `server/services/tradingMonitor.ts`. Currently uses realistic mock data structured for swap to real market feed later.
