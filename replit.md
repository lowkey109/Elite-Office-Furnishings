...
- **Trading AI Monitor** (`/admin/trading-monitor`): Completely separate paper trading engine for crypto (BTC/USD, ETH/USD, SOL/USD) and gold (XAUUSD). **DB-backed lifecycle** with 13 PostgreSQL tables: core (paper_trading_decisions, paper_positions, paper_trade_outcomes, paper_trading_state, market_snapshots), learning (strategy_performance_snapshots, decision_reviews, edge_insights, learning_recommendations), adaptive (trading_config_versions, trading_config_changes, trading_adaptation_proposals, trading_rollbacks). Paper capital starts at $100,000. **Live market feeds** from CoinGecko (BTC, ETH, SOL) — polled every 15s (fast) and 60s (detailed). XAUUSD marked unavailable (no free feed). Real lifecycle: decision → paper position → live mark-to-market from real snapshots → auto-exit (stop/target/max hold) against auditable snapshot IDs → outcome written → performance recalculated from DB. **T005 Learning Engine**: Outcome analysis (by strategy/symbol/confidence/duration/exit reason), edge detection (top/weak setups, degradation, anomalies), strategy health scoring (isDegrading/isHighPerforming), decision quality scoring (0-100 with labels), evidence-based recommendations — all requiring minimum 20 trade outcomes. **T006 Adaptive Execution**: Versioned trading parameters (minConfidence, maxHoldMinutes, enabledStrategies, symbolRiskMultipliers, etc.), rule update engine converts learning into candidate parameter changes, strict guardrails (min sample size 20, max 2 changes/cycle, confidence floor), shadow validation against historical outcomes, config rollback system, review-required approval mode (no auto-apply by default). Service: `server/services/trading/` (modular: types, strategies, marketDataAdapter, marketSnapshots, positionMarker, exitEvaluator, marketLoop, marketContext, newsAdapter, paperEngine, tradingLearning, tradingEdgeAnalysis, strategyHealth, decisionQuality, learningRecommendations, tradingConfig, ruleUpdateEngine, tradingGuardrails, shadowValidation, configRollback, adaptationReport, index). Endpoints: `GET /api/admin/trading/monitor`, `GET /api/admin/trading/learning`, `GET /api/admin/trading/adaptation`.
