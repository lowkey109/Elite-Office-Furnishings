...
- **Trading AI Monitor** (`/admin/trading-monitor`): Completely separate paper trading engine for crypto (BTC/USD, ETH/USD, SOL/USD) and gold (XAUUSD). **DB-backed lifecycle** with 5 PostgreSQL tables: `paper_trading_decisions`, `paper_positions`, `paper_trade_outcomes`, `paper_trading_state`, `market_snapshots`. Paper capital starts at $100,000. **Live market feeds** from CoinGecko (BTC, ETH, SOL) — polled every 15s (fast) and 60s (detailed). XAUUSD marked unavailable (no free feed). Real lifecycle: decision → paper position → live mark-to-market from real snapshots → auto-exit (stop/target/max hold) against auditable snapshot IDs → outcome written → performance recalculated from DB. Market loop runs background polling, writes snapshots to DB, marks open positions from fresh data only (skips stale), evaluates exits with snapshot audit trail (`exitSnapshotId` on positions and outcomes). Stale detection: snapshots flagged stale if `isStale` flag set or age > 120s — positions never marked/exited from stale data. 48h snapshot pruning. Frontend shows FEEDS LIVE badge, real prices with source/timestamp, stale/unavailable states per symbol, data quality percentage. Service: `server/services/trading/` (modular: types, strategies, marketDataAdapter, marketSnapshots, positionMarker, exitEvaluator, marketLoop, marketContext, news, paperEngine, index).
