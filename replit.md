...
- **Trading AI Monitor** (`/admin/trading-monitor`): Completely separate paper trading observation panel for crypto (BTC/USD, ETH/USD, SOL/USD) and gold (XAUUSD). 8 sections: State Cards (9: Mode, Regime, Last Decision, Total Trades, Win Rate, Drawdown, Open Positions, Best Strategy, Data Quality), Market Context (4 assets with price, technicals, support/resistance, funding rate, OI, Fear & Greed), Decision Feed (30 expandable decisions with detailed thesis, entry/exit rules, catalysts, risks, full JSON payload), Open Positions (4 active paper trades), Performance (PnL chart, Total PnL, Sharpe Ratio, Profit Factor, Max Drawdown, Expectancy, Avg Win/Loss, Streaks), Recent Outcomes (20 trades with direction, slippage, duration), Market News & Intelligence (12 categorised news items with sentiment, impact, market tags, summaries), Strategy Knowledge Base (6 strategies: momentum_breakout, mean_reversion, trend_follow, volatility_squeeze, regime_shift, order_flow_scalp — each with entry/exit/invalidation rules, edge, strengths/weaknesses). Read-only, auto-refresh 5s with expanded-row persistence via useRef, 30s cached deterministic data. Backend: `GET /api/admin/trading/monitor` (admin-auth protected). Service: `server/services/tradingMonitor.ts`.
