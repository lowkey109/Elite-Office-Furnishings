...
- **Trading AI Monitor** (`/admin/trading-monitor`): Completely separate paper trading engine for crypto (BTC/USD, ETH/USD, SOL/USD) and gold (XAUUSD). **DB-backed lifecycle** with 4 PostgreSQL tables: `paper_trading_decisions`, `paper_positions`, `paper_trade_outcomes`, `paper_trading_state`. Paper capital starts at $100,000. Real lifecycle: decision → paper position (at real market price) → monitor stops/targets → rule-driven exit → outcome written → performance recalculated from DB. No seeded/fake data — all decisions, positions, outcomes come from DB. Performance (Sharpe, PF, MaxDD) calculated only from actual `paper_trade_outcomes`. Positions stay open/unresolved until real market feeds connect (T003). 8 UI sections: State Cards, Market Context (static until T003), Decision Feed (expandable, from DB), Open Positions (from DB), Performance (from DB outcomes), Recent Outcomes (from DB), Market News (static until T004), Strategy Knowledge Base (6 strategies). Fully typed frontend (no `any`), graceful refresh-failure handling (keeps last good data, shows FEED DELAYED badge). Auto-refresh 5s, expanded-row persistence via useRef. Backend: `GET /api/admin/trading/monitor` (admin-auth protected). Service: `server/services/trading/` (modular: types, strategies, marketContext, news, decisions, positions, outcomes, performance, paperEngine, index).
