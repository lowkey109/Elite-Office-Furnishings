...
- **Autonomous Business Intelligence Layer**: A background job engine (`intelligenceScheduler.ts`) running 5 job types on configurable intervals: System Health Check (every 12h), Spending Trend Analysis (every 24h), Website Issue Detection (every 24h), SEO Blog Article Generation (every 7d), and Weekly Business Report (every 7d). All jobs are recorded in the `scheduled_jobs` DB table with timing, status, and results. Admin access via `/admin/intelligence-hub`.
