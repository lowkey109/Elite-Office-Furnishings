...
- **Trading AI Monitor** (`/admin/trading-monitor`): Paper trading engine for crypto (BTC/ETH/SOL) and gold (XAUUSD). **35 PostgreSQL tables** across 20 stages (T001–T021). Paper capital $100K. Live CoinGecko feeds. **T005**: Learning engine. **T006**: Adaptive execution. **T007**: Portfolio allocation + correlation clusters. **T008**: 16 stress scenarios + resilience. **T009**: Execution quality + slippage. **T010**: Live bridge + dry-run. **T012**: Compliance (8 rules, audit logs, hard limits). **T013**: Cost model (fee/spread/slippage per asset). **T014**: Execution timing + latency. **T015**: Multi-factor signals (price+news+momentum+regime). **T016**: Regime detection (trending/ranging/volatile/risk_off). **T017**: Meta-strategy selection (dynamic enable/disable). **T018**: Capital scaling (drawdown/performance-based sizing). **T019**: System observability (health, error rate, traces). **T020**: Autonomous mode (circuit breakers, safety rules). **T021**: Institutional layer (accounts, partitions, Sharpe, reporting). Endpoints: `/api/admin/trading/monitor`, `/learning`, `/adaptation`, `/portfolio`, `/stress`, `/execution`, `/live-bridge`, `/compliance`, `/cost-model`, `/timing`, `/multi-factor`, `/regime`, `/meta-strategy`, `/capital-scaling`, `/observability`, `/autonomous`, `/institutional`.
