...
const symbol = "registerNexoraHumanApprovedEmailOutboxRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraHumanApprovedEmailOutboxRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with email approval routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora email approval routes into server/routes.ts");
