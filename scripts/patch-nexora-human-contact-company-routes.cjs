...
const symbol = "registerNexoraHumanContactCompanyRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraHumanContactCompanyRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with human-company routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora human-company routes into server/routes.ts");
