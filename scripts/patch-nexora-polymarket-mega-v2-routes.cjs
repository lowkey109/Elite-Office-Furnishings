...
const symbol = "registerNexoraPolymarketMegaAcceleratorV2Routes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraPolymarketMegaAcceleratorV2Routes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with Polymarket mega v2 routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora Polymarket mega v2 routes into server/routes.ts");
