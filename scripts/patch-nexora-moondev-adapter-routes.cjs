...
const symbol = "registerNexoraMoonDevAdapterRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraMoonDevAdapterRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with MoonDev adapter routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora MoonDev adapter routes into server/routes.ts");
