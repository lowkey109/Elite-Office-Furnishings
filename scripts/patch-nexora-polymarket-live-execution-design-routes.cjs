...
const symbol = "registerNexoraPolymarketLiveExecutionDesignRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraPolymarketLiveExecutionDesignRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with live execution design routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora live execution design routes into server/routes.ts");
