...
const symbol = "registerNexoraFinalLocalV1Routes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraFinalLocalV1Routes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with final local v1 routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora final local v1 routes into server/routes.ts");
