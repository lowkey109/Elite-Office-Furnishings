...
const symbol = "registerNexoraSupremeMatrixRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraSupremeMatrixRoutes";
...
if (!s.includes(`${symbol}(server, app)`) && !s.includes(`${symbol}(app)`)) {
  const patches = [
    ...
    },
    {
      ...
      add: `$1\n  try { ${symbol}(typeof server !== "undefined" ? server : app, typeof app !== "undefined" ? app : undefined); } catch (e) { console.error("[NEXORA_SUPREME_ROUTES_PATCH_ERROR]", e); }`,
    },
    {
...
console.log("Patched Build 12 routes into", target);
