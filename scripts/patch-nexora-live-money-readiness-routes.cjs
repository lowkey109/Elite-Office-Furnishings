...
const symbol = "registerNexoraLiveMoneyReadinessRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraLiveMoneyReadinessRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with live money readiness routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora live money readiness routes into server/routes.ts");
