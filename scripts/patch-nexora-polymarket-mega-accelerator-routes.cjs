...
const symbol = "registerNexoraPolymarketMegaAcceleratorRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraPolymarketMegaAcceleratorRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with mega accelerator routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora Polymarket mega accelerator routes into server/routes.ts");
