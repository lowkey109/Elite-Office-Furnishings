...
const symbol = "registerNexoraCompanyRunRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraCompanyRunRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with company-run routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora company-run routes into server/routes.ts");
