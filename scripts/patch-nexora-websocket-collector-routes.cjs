...
const symbol = "registerNexoraWebSocketCollectorRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraWebSocketCollectorRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with collector routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora websocket collector routes into server/routes.ts");
