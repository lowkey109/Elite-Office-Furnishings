...
const symbol = "registerNexoraOfficeAgentAdapterRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraOfficeAgentAdapterRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with office agent adapter routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched office agent adapter routes into server/routes.ts");
