...
const symbol = "registerNexoraPolymarketSuperstackRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraPolymarketSuperstackRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with polymarket superstack routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora Polymarket superstack routes into server/routes.ts");
