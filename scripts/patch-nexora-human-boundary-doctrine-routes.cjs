...
const symbol = "registerNexoraHumanBoundaryDoctrineRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraHumanBoundaryDoctrineRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with human-boundary routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora human-boundary doctrine routes into server/routes.ts");
