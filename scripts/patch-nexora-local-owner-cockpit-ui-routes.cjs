...
const symbol = "registerNexoraLocalOwnerCockpitUiRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraLocalOwnerCockpitUiRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with local UI routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora local owner cockpit UI routes into server/routes.ts");
