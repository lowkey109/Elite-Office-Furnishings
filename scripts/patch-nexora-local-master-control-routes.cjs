...
const symbol = "registerNexoraLocalMasterControlRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraLocalMasterControlRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with local master routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora local master routes into server/routes.ts");
