...
const symbol = "registerNexoraPolymarketRealCollectorRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraPolymarketRealCollectorRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with Polymarket collector routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora Polymarket real collector routes into server/routes.ts");
