...
const symbol = "registerNexoraTradingIntelligenceLabV2Routes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraTradingIntelligenceLabV2Routes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with trading lab routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora trading lab v2 routes into server/routes.ts");
