...
const symbol = "registerNexoraResearchBridgeRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraResearchBridgeRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with research bridge routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora research bridge routes into server/routes.ts");
