...
const symbol = "registerNexoraAlmightyRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraAlmightyRoutes";
...
if (!s.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with almighty routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, s);
console.log("Patched Nexora almighty routes into server/routes.ts");
