...
const symbol = "registerNexoraOfficeFurnitureAgentRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraOfficeFurnitureAgentRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with office furniture agent routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched office furniture agent routes into server/routes.ts");
