...
const symbol = "registerNexoraUnifiedAgentRuntimeRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraUnifiedAgentRuntimeRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with unified runtime routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora unified agent runtime routes into server/routes.ts");
