...
const symbol = "registerNexoraExtensionManagerRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraExtensionManagerRoutes";
...
if (!s.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with extension manager routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, s);
console.log("Patched Nexora extension manager routes into server/routes.ts");
