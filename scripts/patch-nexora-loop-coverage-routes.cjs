...
const symbol = "registerNexoraLoopCoverageRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraLoopCoverageRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with loop coverage routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora loop coverage routes into server/routes.ts");
