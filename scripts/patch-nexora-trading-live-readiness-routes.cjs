...
const symbol = "registerNexoraTradingLiveReadinessGateRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraTradingLiveReadinessGateRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with trading readiness routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora trading readiness routes into server/routes.ts");
