...
const symbol = "registerNexoraProductImportExportRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraProductImportExportRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with product import/export routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora product import/export routes into server/routes.ts");
