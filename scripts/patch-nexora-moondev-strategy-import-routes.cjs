...
const symbol = "registerNexoraMoonDevStrategyBacktestImporterRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraMoonDevStrategyBacktestImporterRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with MoonDev strategy import routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora MoonDev strategy import routes into server/routes.ts");
