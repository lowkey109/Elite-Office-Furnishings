...
const symbol = "registerNexoraProductionReadinessRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraProductionReadinessRoutes";
...
if (!s.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with production readiness routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, s);
console.log("Patched Nexora production readiness routes into server/routes.ts");
