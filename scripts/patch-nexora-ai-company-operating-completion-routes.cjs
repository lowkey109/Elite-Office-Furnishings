...
if (!source.includes(`${symbol}(app);`)) {
  const patterns = [
    /(export\s+async\s+function\s+registerRoutes\s*$begin:math:text$\[\^\)\]\*$end:math:text$\s*(?::[^{]+)?\s*{)/,
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with completion routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora AI company operating completion routes into server/routes.ts");
