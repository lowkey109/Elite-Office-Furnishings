...
const symbol = "registerNexoraTradingExecutionSafetyRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraTradingExecutionSafetyRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with trading execution routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora trading execution safety routes into server/routes.ts");
