...
const symbol = "registerNexoraAdvancedLocalOpsRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraAdvancedLocalOpsRoutes";
...
if (!s.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with advanced local ops routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, s);
console.log("Patched Nexora advanced local ops routes into server/routes.ts");
