...
const symbol = "registerNexoraPolyFivePackRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraPolyFivePackRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with poly five pack routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora poly five pack routes into server/routes.ts");
