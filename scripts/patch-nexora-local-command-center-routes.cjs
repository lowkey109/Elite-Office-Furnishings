...
const symbol = "registerNexoraLocalCommandCenterRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraLocalCommandCenterRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with local command center routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora local command center routes into server/routes.ts");
