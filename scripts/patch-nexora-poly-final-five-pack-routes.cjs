...
const symbol = "registerNexoraPolyFinalFivePackRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraPolyFinalFivePackRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with poly final five pack routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora Poly Final 5 Pack routes into server/routes.ts");
