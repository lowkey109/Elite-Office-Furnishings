...
const symbol = "registerNexoraMegaBuildRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraMegaBuildRoutes";
...
if (!s.includes(`${symbol}(server, app)`) && !s.includes(`${symbol}(app)`)) {
  const patches = [
    ...
    },
    {
      ...
      add: `$1\n  try { ${symbol}(typeof server !== "undefined" ? server : app, typeof app !== "undefined" ? app : undefined); } catch (e) { console.error("[NEXORA_MEGA_ROUTES_PATCH_ERROR]", e); }`,
    },
    {
...
console.log("Patched Mega Build routes into", target);
