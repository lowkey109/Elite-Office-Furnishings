...
const symbol = "registerNexoraPaperAutopilotEvidenceRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraPaperAutopilotEvidenceRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with paper autopilot routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora paper autopilot routes into server/routes.ts");
