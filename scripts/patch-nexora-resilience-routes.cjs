...
const symbol = "registerNexoraResilienceRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraResilienceRoutes";
...
if (!s.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with resilience routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, s);
console.log("Patched Nexora resilience routes into server/routes.ts");
