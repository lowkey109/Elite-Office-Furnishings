...
const symbol = "registerNexoraPolyNextFivePackRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraPolyNextFivePackRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with poly next five pack routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora Poly Next 5 Pack routes into server/routes.ts");
