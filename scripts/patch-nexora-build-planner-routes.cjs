...
const symbol = "registerNexoraBuildPlannerRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraBuildPlannerRoutes";
...
if (!s.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with build planner routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, s);
console.log("Patched Nexora build planner routes into server/routes.ts");
