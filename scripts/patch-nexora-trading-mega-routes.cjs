...
const symbol = "registerNexoraTradingMegaRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraTradingMegaRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with trading mega routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora trading mega routes into server/routes.ts");
