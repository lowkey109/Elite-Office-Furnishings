...
const symbol = "registerNexoraPolymarketFinalHardeningRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraPolymarketFinalHardeningRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with Polymarket final routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora Polymarket final routes into server/routes.ts");
