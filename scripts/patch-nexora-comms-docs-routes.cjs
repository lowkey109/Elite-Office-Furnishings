...
const symbol = "registerNexoraCommsDocsRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraCommsDocsRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with comms docs routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora comms docs routes into server/routes.ts");
