...
const symbol = "registerNexoraMoonDevPhase1Routes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraMoonDevPhase1Routes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with MoonDev Phase1 routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora MoonDev Phase1 routes into server/routes.ts");
