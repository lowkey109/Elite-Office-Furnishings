...
const symbol = "registerNexoraLocalActionExecutorRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraLocalActionExecutorRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with local executor routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora local action executor routes into server/routes.ts");
