...
const symbol = "registerNexoraPolymarketLocalOperatorUiRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraPolymarketLocalOperatorUiRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with Polymarket UI routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora Polymarket UI routes into server/routes.ts");
