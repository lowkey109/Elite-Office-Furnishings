...
const symbol = "registerNexoraAICompanyV1HardeningRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraAICompanyV1HardeningRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with v1 hardening routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora AI Company v1 hardening routes into server/routes.ts");
