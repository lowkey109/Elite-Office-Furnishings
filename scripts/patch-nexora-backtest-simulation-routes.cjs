...
const symbol = "registerNexoraBacktestSimulationRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraBacktestSimulationRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with backtesting routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora backtesting routes into server/routes.ts");
