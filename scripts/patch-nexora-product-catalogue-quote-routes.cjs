...
const symbol = "registerNexoraProductCatalogueQuoteRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraProductCatalogueQuoteRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with product catalogue routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora product catalogue quote routes into server/routes.ts");
