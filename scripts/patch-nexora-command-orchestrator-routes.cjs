...
const symbol = "registerNexoraCommandOrchestratorRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraCommandOrchestratorRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with command orchestrator routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora command orchestrator routes into server/routes.ts");
