...
const symbol = "registerNexoraActiveLocalLoopDaemonRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraActiveLocalLoopDaemonRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with active loop routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora active local loop daemon routes into server/routes.ts");
