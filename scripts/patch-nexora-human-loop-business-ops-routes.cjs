...
const symbol = "registerNexoraHumanLoopBusinessOpsRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraHumanLoopBusinessOpsRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with human ops routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora human-loop business ops routes into server/routes.ts");
