...
const symbol = "registerNexoraMoonDevSystemsAcceleratorRoutes";
const routeFile = "server/services/intelligence/nexora/autonomy/routes/nexoraMoonDevSystemsAcceleratorRoutes";
...
if (!source.includes(`${symbol}(app);`)) {
  ...
  if (!patched) {
    console.error("Could not patch registerRoutes with MoonDev systems routes.");
    process.exit(1);
  }
}

fs.writeFileSync(target, source);
console.log("Patched Nexora MoonDev systems routes into server/routes.ts");
